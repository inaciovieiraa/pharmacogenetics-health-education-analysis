...
dadospre_sujos <- read.csv("data/Dados_pre_farmacia.csv")
...
ggplot(resumo_q2, aes(x = q2_medico, y = n)) +
  ...
  labs(
    x = "NIVEL DE CONCORDÂNCIA",
    y = "QUANTIDADE DE ALUNOS",
    title = "RESPONSABILIDADE DO FARMACÊUTICO: Visão da turma antes da aula",
    subtitle = "A investigação de riscos genéticos e a definição da terapia personalizada\nsão responsabilidades exclusivas da equipe médica, cabendo ao \nfarmacêutico apenas a dispensação da medicação prescrita.") + 
  geom_text(aes(label = paste0(round(porcentagem, 1), "%")),
            hjust = 0.5,
            size = 4.3,
            vjust = -0.3,
